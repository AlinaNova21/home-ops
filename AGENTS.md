...
- **namespace**: Kubernetes namespace (`downloads/`, `entertainment/`, etc.)
...
3. No `**/ns.yaml` outside namespace level (except `bootstrap/` and `infrastructure/`)
...
```
kubernetes/
├── apps/                    # Application deployments (per-namespace kustomization)
│   └── {namespace}/
│       ├── ns.yaml
│       ├── kustomization.yaml
│       └── {component}/
...
│   ├── auth/                # dex (external/internal) + security-policies
│   ├── cert-manager/
│   ├── cnpg-system/         # CloudNativePG + barman plugin
│   ├── external-secrets-system/
│   ├── flux-system/         # webhook receiver
│   ├── headlamp/
│   ├── inteldeviceplugins-system/
│   ├── kopiur-system/
│   ├── kube-system/         # cilium, descheduler, gvisor, metrics-server, snapshot-controller
...
├── flux-config/             # Flux CD self-management (HelmRelease, OCIRepository, root kustomizations)
...
| `just flux-sync` | Annotate OCIRepository + reconcile infrastructure + apps |
...
**CI** runs in `.github/workflows/validate-kubernetes.yml`: `kustomize build` + `kubeconform` per directory (`flux-config/`, `infrastructure/`, `apps/`).
...
kustomize build kubernetes/apps | kubeconform \
  -strict \
  -ignore-missing-schemas \
  -schema-location default \
  -schema-location 'https://raw.githubusercontent.com/datreeio/CRDs-catalog/main/{{.Group}}/{{.ResourceKind}}_{{.ResourceAPIVersion}}.json'

# Or all three top-level dirs in a loop
for dir in kubernetes/flux-config kubernetes/infrastructure kubernetes/apps; do
  echo "=== $dir ==="
